"""
Test: Donchian Channel Breakout - NAYI strategy (pehli baar test ho rahi
hai). Idea #2 jo pehle recommend ki gayi thi.

Idea: Classic "Turtle Trading" style breakout - jab close, pichli N candles
ki highest-high se UPAR band ho (N-period Donchian Channel breakout), to
naya uptrend/momentum move shuru samjha jata hai. Ye is session ke
established sabaq (breakout/trend-continuation kaamyab, mean-reversion
nakam) ke bilkul seedhe mutabiq hai - shayad sab se "purest" breakout idea.

4 buckets:
  A) Donchian 20-period - Baseline (koi naya filter nahi) [classic turtle short-term]
  B) Donchian 20-period - + Naye Filters (ETH Regime + RS Trend + RS%95)
  C) Donchian 55-period (zyada lamba/mazboot breakout) - Baseline [classic turtle long-term]
  D) Donchian 55-period - + Naye Filters
...
hua (chhoti-muddat/quick-reentry trades ko harm karta hai). Yahan is baat
ka khaas khayal hai kyunke Donchian khud bhi ek "naye high" wala idea hai
- 52W filter dono ek doosre ke sath overlap/conflict kar sakte hain.

Top 150 coins, 1h. Exit: Chandelier (period=16, multiplier=4.5).

Chalayen: python test_donchian_channel_breakout.py
"""
...
DONCHIAN_PERIOD_FAST = 20   # classic turtle short-term system
DONCHIAN_PERIOD_SLOW = 55   # classic turtle long-term system
...
def donchian_channel_breakout(df, channel_period):
    ...
    # pichli N candles ki highest high (current candle shamil nahi)
...
def main():
    ...
    for symbol in coins:
        ...
        try:
            sig_fast = apply_cooldown(donchian_channel_breakout(df, DONCHIAN_PERIOD_FAST), config.SIGNAL_COOLDOWN_BARS)
            sig_slow = apply_cooldown(donchian_channel_breakout(df, DONCHIAN_PERIOD_SLOW), config.SIGNAL_COOLDOWN_BARS)
        except Exception as e:
            print(f"[{done}/{len(coins)}] {symbol}: Donchian calc fail ({e}), skip")
            continue
    ...
    emit("\n\n########## DONCHIAN CHANNEL BREAKOUT - RESULTS ##########")
    summarize(buckets["A"], f"A) Donchian {DONCHIAN_PERIOD_FAST}-period - Baseline")
    summarize(buckets["B"], f"B) Donchian {DONCHIAN_PERIOD_FAST}-period - + Naye Filters (ETH+RS+RS%95)")
    summarize(buckets["C"], f"C) Donchian {DONCHIAN_PERIOD_SLOW}-period - Baseline")
    summarize(buckets["D"], f"D) Donchian {DONCHIAN_PERIOD_SLOW}-period - + Naye Filters")

    result_file = "test_donchian_channel_breakout_RESULTS.txt"
